...
class Player extends CI_Controller {
    ...
    public function index($album_id = null) {
        ...
        if ($this->input->get('song')) {
            ...
            // Load the album containing this song so autoplay can find it
            if ($data['autoplay_song']) {
                $album_link = $this->db->select('album_id')
                    ->where('song_id', $this->input->get('song'))
                    ->get('album_songs')->row();
                if ($album_link) {
                    $data['current_album'] = $this->Album_model->get_album_with_songs($album_link->album_id);
                } else {
                    // Song is misc — load misc as the current album
                    $misc_songs = $this->Song_model->get_misc_songs();
                    $data['current_album'] = (object)[
                        'id' => 'misc',
                        'title' => 'Misc Songs',
