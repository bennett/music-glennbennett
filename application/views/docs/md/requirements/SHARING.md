...
## Song Deep Links (`?song=` Parameter)
When a user clicks a shared link like `/?song=1`, the Player controller must load the album that contains the song so the track list includes it. Songs can belong to a named album or be **misc songs** (songs with no album tag, or albums with fewer than `min_album_songs` tracks — see [Misc Album](../technical/ADMIN.md#misc-album)). Both are normal states.

The Player controller:
1. Fetches the song from the database for OG tags
2. Looks up the song's album via the `album_songs` table
3. If found in an album, loads that album as `initialAlbum`
4. If the song is a misc song (not linked to any album), builds a Misc Songs pseudo-album containing all misc songs and loads that as `initialAlbum`

This ensures `autoplaySong()` can find the song in the loaded track list. Without this, the player would load whatever album was last played (or the default), and the song wouldn't be in the track list.
