...
<div class="tools-grid">
    ...
    <div class="card tool-card">
        ...
        <p>Clean old play history. Keeps last 90 days by default.</p>
        <form method="post" action="<?= site_url('admin/cleanup_history') ?>" style="margin-top:16px;" onsubmit="return confirm('This will delete play history older than 90 days. Continue?');">
            <button type="submit" class="btn btn-secondary">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 9h6v6H9z"/></svg>
                Clean Old History
            </button>
        </form>
        <?php if (isset($play_history_count)): ?>
        <div class="tool-meta"><?= number_format($play_history_count) ?> total play records</div>
        <?php endif; ?>
    </div>
    ...
    <div class="card tool-card">
        <div class="tool-icon">🖼️</div>
        <h3>Share Images</h3>
        <p>Preview all generated Facebook/Twitter share images for songs and albums.</p>
        <a href="<?= site_url('admin/share_images') ?>" class="btn btn-secondary" style="margin-top:16px;display:inline-flex;">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
            View All Images
        </a>
    </div>
