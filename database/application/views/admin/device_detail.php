...
foreach ($plays as $p) {
    if (!empty($p->played_at)) {
        $hour = (int)date('G', strtotime($p->played_at));
        $day = date('l', strtotime($p->played_at));
...
$engaged_plays = 0;
foreach ($plays as $p) {
    if (($p->percent_played ?? 0) >= 50) $engaged_plays++;
}
...
<details class="card compact" open>
    <summary class="card-summary">
        ...
        <span class="summary-meta"><?= $device->last_seen ? 'Last seen ' . date('M j', strtotime($device->last_seen)) : '' ?></span>
    </summary>
    <div class="card-details">
        ...
        <div class="detail-row">
            ...
            <span class="detail-value"><?= $device->first_seen ? date('M j, Y', strtotime($device->first_seen)) : '—' ?></span>
        </div>
        <div class="detail-row">
...
<div class="card compact">
    ...
    <div class="mini-chart">
        ...
        ?>
        <div class="mini-bar" style="height:<?= $height ?>px;background:<?= $bg ?>" title="<?= date('M j', strtotime($date)) ?>: <?= $count ?>"></div>
        <?php endfor; ?>
    </div>
</div>
<?php endif; ?>
...
<div class="card compact">
    ...
    <div class="song-row-compact">
        ...
        <div class="fav-date"><?= $fav->favorited_at ? date('M j', strtotime($fav->favorited_at)) : '' ?></div>
    </div>
    <?php endforeach; ?>
</div>
<?php endif; ?>
...
<?php if (!empty($plays)): ?>
<div class="card compact">
    <div class="card-title-sm">▶️ Recent Activity</div>
    <?php foreach (array_slice($plays, 0, 10) as $p): 
        $pct = $p->percent_played ?? 0;
    ...
    <div class="activity-row">
        ...
        <div class="activity-meta">
            ...
            <span class="activity-time"><?= $p->played_at ? date('M j g:ia', strtotime($p->played_at)) : '' ?></span>
        </div>
    </div>
    <?php endforeach; ?>
    <?php if ($total_plays > 10): ?>
    <div class="more-link">+ <?= $total_plays - 10 ?> more plays</div>
    <?php endif; ?>
</div>
<?php endif; ?>
